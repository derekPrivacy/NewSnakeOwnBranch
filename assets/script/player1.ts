...
export default class NewClass extends cc.Component {
    ...
    start() {
        ...
        // anim.play('test')
        // Object.defineProperty(player1, 'score_player1', {
        //     get: () => this.score_player1,
        //     set: value => {
        //         this.score_player1 = value
        //         this.label_player1.string = `PLAYER 1 \n 得分：${this.score_player1}`
        //     }
        // })

        /* Object.defineProperty(player2, 'score_player2', {
            get: () => score_player2,
            set: value => {
                score_player2 = value
        ...
        this.init()
        cc.systemEvent.on(cc.SystemEvent.EventType.KEY_DOWN, this.onKeyDown_player1.bind(this))
    }

    onKeyDown_player1(event) {
    ...
    move() {
        ...
        const point_player1 = { x: last[0], y: last[1] }
        let type
        ...
        // 判断转向
        if (
            (direction[0].indexOf(type) !== -1 && direction[1].indexOf(this.direction_player1) !== -1)
            ||
            direction[1].indexOf(type) !== -1 && direction[0].indexOf(this.direction_player1) !== -1
        ) {
            type = this.direction_player1
        }
        switch (type) {
        ...
        for (let i = 0; i < map.length; i++) {
            if (point_player1.x === map[i][0] && point_player1.y === map[i][1]) {
        ...
        this.addPoint(point_player1.x, point_player1.y)
    }
    ...
    // 插入点
    addPoint(x: number, y: number, color: cc.Color = new cc.Color(0, 0, 255)) {
        const baseWidth = 720
        const name = x + '_' + y
